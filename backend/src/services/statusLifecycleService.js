...
class StatusLifecycleService {
  constructor({ recordStore = new Map(), statusConfiguration, auditStore = [], authorize = StatusLifecycleService.defaultAuthorize } = {}) {
  ...
  transition({ principal, entityType, entityId, toCode, reason = null, metadata = {} }) {
    ...
    const allowed = this.statusConfiguration.getAllowedTransitions({ principal, entityType, fromCode: record.status }).some(status => status.code === toCode);
    ...
    this.auditStore.push(new AuditEvent({
      ...
      organizationId: principal.organizationId, userId: principal.userId, action: "status.transitioned",
      entityType, entityId, previousValue: { status: previousStatus }, newValue: { status: toCode, reason, metadata }
    }));
    return record;
  }
  ...
  #key(organizationId, entityType, entityId) { return [organizationId, entityType, entityId].join(":"); }
