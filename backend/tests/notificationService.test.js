...
const intruder={userId:"user-c",organizationId:"org-a",permissions:["notification:read","notification:acknowledge","notification:dismiss"]};
...
assert.equal(service.listForRecipient({principal:principalB}).length,1);
assert.equal(service.listForRecipient({principal:intruder}).length,0);
assert.throws(()=>service.markRead({principal:principalB,notificationId:"n1"}),/Not authorized/);
assert.throws(()=>service.markRead({principal:intruder,notificationId:"n1"}),/Not authorized/);
